...
# https://codeshare.io/BA203
    ...
    # Bu metot obje tanımlamaya gerek kalmaksızın sınıf üzerinden kullanılabilir!!!
    # Student.Search(parametreler...)


    @classmethod 
    def Search(cls,fullname="",email="",phonenumber=""):

        foundStudents = list()

        for dictionary in cls.db.values():
            if(fullname!=""):
                if(dictionary["fullname"] == fullname):
                    foundStudents.append(dictionary)
            if(email!=""):
                if(dictionary["email"]==email):
                    foundStudents.append(dictionary)
            if(phonenumber!=""):
                if(dictionary["phonenumber"]==phonenumber):
                    foundStudents.append(dictionary)
    ...
    @classmethod
    def PrintStudentsCount(cls):
        print(f"There is/are {cls.StudentCount} student/s in class")
